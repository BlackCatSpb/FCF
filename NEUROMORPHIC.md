...
VSA-операции FCF — не набор трюков, а **алгебра ℝ[G] на группе ℤ₈^d** (групповая алгебра конечной абелевой группы). Все операции имеют физическую реализацию:

| Операция | Групповая алгебра | Физический смысл |
|----------|-------------------|------------------|
| **bind** (⊙) | Свёртка на G = умножение в частотной области | FFT → поточечное × → IFFT |
| **hybrid bind** | `α·conv + (1-α)·ew` | Среднее свёртки и произведения — два режима в одном |
| **unbind** (⊘) | Корреляция на G | `conj(FFT(a)) · FFT(b)` → IFFT |
| **permute** (ρ) | Сдвиг на элемент группы: `(τ_g v)(h) = v(h-g)` | Циклический сдвиг — O(1) |
| **bundle** (+) | Сумма функций на G | Сложение токов — O(1) |
| **convolution** | `(k*v)(g) = Σ_h k(h)v(g-h)` | Group convolution с custom ядром |

На чипе с микрокодом групповой алгебры:
- **FFT на оси ℤ_s** — `s·log s` операций, s ≤ 8 (фиксированный малый размер)
- **Bind** = FFT → element-wise × → IFFT — конвейер из трёх инструкций
- **Permute** = физический циклический сдвиг — один такт
- **Bundle** = суммирование токов в мемристорном кроссбаре — один такт

Когда статистическая n-граммная решётка даёт менее 3 кандидатов, FCF делает разовый unbind (корреляцию на группе) — на чипе один FFT-конвейер вместо O(V) полного перебора.

**VSACNN** — иерархическая архитектура из VSAConvLayer, каждый слой делает multi-scale group convolution с различными ядрами (gaussian, laplacian, gabor, dog). Результат — bundle scale-специфичных признаков. Без единого learned weight: вся операция — closed-form FFT-свёртка. На нейроморфном чипе это потоковая архитектура: конвейер FFT → convolve → bundle.

**ResidueEncoder** — кодирование чисел через RNS: каждое число → кортеж остатков по модулям 17, 19, 23, …, bind базовых векторов остатков. Арифметика в VSA: add = bundle, mul = bind. Позволяет нейроморфному чипу работать с числовыми данными без числовых ALU.
...
| **Алгебра групп ℤ₈^d** | Все VSA-операции — closed-form group algebra. Не чёрный ящик, а явное преобразование |
| **FFT на оси ℤ_s** | s·log s операций, s ≤ 8 — конвейеризуется в потоковой архитектуре |
| **Custom kernels** | Gaussian, laplacian, gabor, dog — как аппаратные фильтры в оптическом или мемристорном сопроцессоре |
| **ResidueEncoder (RNS)** | Числовые данные → VSA-арифметика без числовых ALU |
