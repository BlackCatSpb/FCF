...
class UnifiedMultidimensionalTransformerV2(nn.Module):
    ...
    def generate_text(self, prompt_ids, cv, max_new=128, temperature=0.8,
                      use_attractors=False, **kwargs):
        """Generation compatible with CharacterVocab/BPEVocab interface.
        
        If use_attractors=True, uses AttractorField.nxt_direction()
        to predict next coordinate instead of boundary_predictor.
        """
        ...
        with torch.no_grad():
            for _ in range(max_new):
                ...
                z_curr = h[0, -1]
                if use_attractors and self.attractor_field.n_attractors > 0:
