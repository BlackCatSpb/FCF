...
# ─── Compact Track Frame ────────────────────────────────────
# 10 полей = 16 байт на шаг (против 1536 байт для [384] float32)
# Сжатие в 96 раз, полная обратимость через pack_token().
...
class CoordinatePacker:
    ...
    def compress_track(self, trajectory: np.ndarray) -> np.ndarray:
        """
        Сжать [L, 384] трек в компактный структурированный массив.
        Из каждого шага извлекаются 10 полей метаданных (97 бит).
        Зарезервированные dims 97-383 теряются — они перевычислимы.
        """
        L = trajectory.shape[0]
        compact = np.empty(L, dtype=TRACK_DTYPE)
        for t in range(L):
    ...
    def convert_generation_compact(self, gen_compact: np.ndarray,
                                    text_id: int = 0) -> np.ndarray:
        """
        Конвертировать 7-польный compact из генерации в полный TRACK_DTYPE.
        meta_type и context_hash вычисляются автоматически.
        """
        L = len(gen_compact)
        ids = [int(gen_compact[t]['token_id']) for t in range(L)]
        full = np.empty(L, dtype=TRACK_DTYPE)
        for t in range(L):
            f = gen_compact[t]
            tid = int(f['token_id'])
            meta_type = self._get_meta_type(
                tid, int(f['word_num']), int(f['pos_in_word']))
            ctx_hash = self._context_hash(ids, t)
            full[t] = (
                tid,
                int(f['pos_in_word']),
                int(f['word_len']),
    ...
    def compress_ids(self, ids, word_boundaries=None,
                     sent_start=0, sent_end=None,
                     text_id=0) -> np.ndarray:
        """
        Собрать компактный трек напрямую из ID токенов (без [L, 384]).
        Возвращает массив TRACK_DTYPE.
        """
        L = len(ids)
        if sent_end is None:
            sent_end = L - 1
    ...
    def decompress_track(self, compact: np.ndarray,
                         fill_reserved_fn=None, **fill_kwargs) -> np.ndarray:
        """
        Восстановить [L, 384] трек из компактного формата.
        Вызывает pack_token() для каждого шага.
        Опционально: fill_reserved_fn(coord, t, compact) для заполнения dims 97-383.
        """
        L = len(compact)
        ...
        for t in range(L):
            frame = compact[t]
            h = self.pack_token(
                token_id=int(frame['token_id']),
                pos_in_word=int(frame['pos_in_word']),
                word_len=int(frame['word_len']),
                word_num=int(frame['word_num']),
                pos_in_sent=int(frame['pos_in_sent']),
                sent_len=int(frame['sent_len']),
                flags=int(frame['flags']),
                meta_type=int(frame['meta_type']),
                context_hash=int(frame['context_hash']),
                text_id=int(frame['text_id']),
            )
            if fill_reserved_fn is not None:
                h = fill_reserved_fn(h, t, compact)
            trajectory[t] = h
        return trajectory
