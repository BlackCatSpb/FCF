...
class AutoTrainer:
    ...
    def __init__(
        ...
        self.finetune_steps: int = 200  # Увеличено со 100

        self._finetune_optimizer = None
        self._finetune_scheduler = None
    ...
    def _retrain_domain(self, domain_id: str, rule):
        ...
        blocks = []
        for fact in facts[:200]:  # Увеличено со 100
            text = (
                f"{fact.get('concept', '')} "
                f"{fact.get('relation', '')} "
                f"{fact.get('target', '')}"
            ...
            if len(blocks) >= 50:  # Увеличено с 20
                break
        ...
        for step in range(self.finetune_steps):
            ...
            for input_ids, labels in blocks[:4]:  # Увеличено с 2
    ...
    def _finetune_on_queries(self, queries: list, steps: int):
        ...
        if self._finetune_optimizer is None:
            self._finetune_optimizer = torch.optim.AdamW(
                self.layer.parameters(), lr=1e-5, weight_decay=0.01, betas=(0.9, 0.95)
            )
            self._finetune_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self._finetune_optimizer, T_max=steps
            )
        optimizer = self._finetune_optimizer
        scheduler = self._finetune_scheduler

        blocks = []
        for fq in queries[:100]:  # Увеличено с 50
