...
Pure symbolic geometry + accumulated role statistics.

Flow:
  words → project to concept space → core_score each word →
  weighted centroid → semantic attractor → modifier field
...
class SemanticGate:
    ...
    def _resolve_all(self, words):
        ...
        for w in words:
            ...
            if v is not None and conf > 0:
                anchors[w] = {'cid': cid, 'conf': conf, 'vec': v.copy()}
        return anchors
    ...
    def _core_score(self, word, cid, anchors=None):
        ...
        return (role_ratio * 0.5 + freq_norm * 0.2 +
